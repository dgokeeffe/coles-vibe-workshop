...
> **Scope:** You are inside `starter-kit/`. This is the participant scaffold — build the lab here. Do not edit files outside this directory. Sibling codebases (`reference-implementation/`, `quiz-app/`) exist but are out of scope for this session — `reference-implementation/` is the facilitator answer key and must not be referenced or copied from during labs.
