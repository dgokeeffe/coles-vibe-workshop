...
**Why this matters:** You've been _using_ MCP servers all day. Now you'll _build_ one — the same pattern Coda could use to expose internal data to every agent in the organisation.
...
Now any agent in the org can query Coda retail data via MCP — no custom integration code.
