...
**Audience:** Coda — Data & AI Engineering Team
...
- Prize ready for demo winners (announced at close)
...
> **Arc:** Set frame before any tool. Not "AI makes you 30% faster" — "verification is the new bottleneck." Show the 6-hours-from-now destination.
...
- Welcome — thanks to your platform champions for organising. Introduce yourself briefly.
...
**Transition:** "Before any theory — a quick frame on where the industry is right now."
...
**Transition:** "Let me show you what today ships."

> **NOTE:** If the room looks sceptical, keep the METR "felt 20% faster, were 19% slower" data point in reserve — do not open with it.

---

## Slide 3: Six Hours From Now | 10:02 | 90 sec **[NEW]**
...
- Walk each pane in one line — DE (Bronze → Silver → Gold + quality gates), DS (feature pipeline + MLflow experiment + UC-registered model + batch predictions), Analyst (Genie + dashboard + embedded FastAPI).
...
- "For Coda: wrap internal APIs, data tools, monitoring behind MCP servers — every agent gets access."
...
- **[R.V.P.I. FRAME]** "These are all V automation. Hooks check before the plan. Skills standardise the verified patterns. This is reproducible — set up the same at Coda."
...
> **Pre-lunch:** flag: "Come back at 13:00 — pair formation and Lab 1 briefing."
...
- Walk the three track cards:
  ...
  - **Data Science** (purple) — feature engineering, MLflow experiment tracking, correlation analysis + EDA. Capability: MLflow + Unity Catalog model registry (no Model Serving this lab — Lab 2 uses batch inference). `LAB-1-DS.md` · `track-ds.html`.
...
# BLOCK 7 — Show & Tell (14:45–15:00)

> **Arc:** One highlight per track in 15 min. 30 sec each — not a full demo. Closes the morning's loop.

---

## Slide 36: Show and Tell | 14:45 | 15 min

**[CLICK]**

**[ENERGY]** "Pencils down. Fifteen minutes."
...
- Optional: query a couple of Lab 1 Gold tables live to show the room what the Bronze→Gold flow produced in practice.
...
- Walk the three track cards:
  ...
  - **Data Science** — train forecasting model, register in Unity Catalog with `@champion` alias, score a sample in a notebook (Tier 1 default). Stretch for fast pairs: Lakeflow batch job using `mlflow.spark_udf`. No Model Serving endpoint in this lab. `LAB-2-DS.md`.
...
- **Coming soon:** broader team rollout, shared skill libraries for common Coda patterns, Genie spaces on your real production data.
- **Champions:** your platform champions (internal), david.okeeffe@databricks.com (Databricks SA — available for follow-up).
...
- "Coda sits on the right today — external agent connecting via MCP, same security and governance."
...
- **For Coda:** fork this repo, add custom skills — `/run-data-quality`, `/deploy-pipeline`, `/check-lineage`.
