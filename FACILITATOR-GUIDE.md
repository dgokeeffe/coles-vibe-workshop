...
>
> This document uses an older slide numbering (no AI Coding Moment, no current CLAUDE.md slides) and has not been kept in sync with the deck.
>
...
- **Speaker Notes:**
  ...
  - Thank the Coda Data & AI Engineering team for hosting
...
- **Speaker Notes:**
  ...
  - Coda callout: today you will use Lakeflow, Unity Catalog, Genie, AI/BI, Custom Apps, MCP, and Model Serving
...
- **Facilitator Tips:** Use the image as a visual anchor. Don't try to explain every box -- hit the highlights and say "we will touch most of these today." The Coda callout at the bottom is a good moment to connect the abstract platform to their concrete work.
...
4. *What Coda-specific standards would you add through conversation?* -- Data governance rules, approved libraries, naming conventions
...
- **Facilitator Tips:** "Hands on keyboards. You have 65 minutes. The lab guide for your track is in `LAB-1-DE.md` / `LAB-1-DS.md` / `LAB-1-ANALYST.md`. Start with Phase 1: Write Tests First. Go!"
...
- **Speaker Notes:**
  ...
  - For Coda: dozens of internal tools -- without MCP you would write custom code for each one
...
- **Time:** ~1:19 PM
- **Duration:** ~4 min
- **Key Message:** Client-server architecture with JSON-RPC. MCP handles wiring, skills handle knowledge, the agent orchestrates both. For Coda: one protocol wraps Unity Catalog, internal APIs, and Genie.
- **Speaker Notes:**
  ...
  - For Coda: an MCP server wrapping Unity Catalog means every agent in the org can query tables without custom code
...
- **Speaker Notes:**
  ...
  - Coda callout: in Lab 2 you will connect your app to Genie via a managed MCP server
...
- **Facilitator Tips:** Three cards, three flavours. Quick summary: "Managed = Databricks gives you. Proxy = connect third-party. Custom = build your own." The Coda callout at the bottom connects this to Lab 2.
...
- **Speaker Notes:**
  ...
  - Coda would be on the right side today -- an external agent connecting into Databricks via MCP
...
- **Speaker Notes:**
  ...
  - Coda callout: fork the AI Dev Kit and add custom skills (/run-data-quality, /deploy-pipeline, /check-lineage)
...
- **Key Message:** Databricks internally configures Claude Code with CLAUDE.md, hooks, skills, and MCP servers. This setup is reproducible for Coda.
- **Speaker Notes:**
  ...
  - Key insight: this is reproducible. Coda can set up the same structure.
- **What to Watch For:**
...
- **Time:** ~3:03 PM
- **Duration:** ~3 min
- **Key Message:** Concrete actions: share learnings, set up a team CLAUDE.md, try it on a real task this week. Champions: your platform champions (internal), David (Databricks).
- **Speaker Notes:**
  ...
  - Champions: your platform champions (internal), David O'Keeffe (Databricks)
...
**"What Coda-specific standards should be in a shared team CLAUDE.md?"**
